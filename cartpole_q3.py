...
while True:
    ...
    for i in xrange(200):
        ...
        #Now train
        if len(memory) == 1000:             #Start Training when memory hass 1000 population
            batch_num = 32
            flag = 1
        #else:
        ...
        if flag==1:
            ...
            target_vector = np.zeros((batch_num,2))   #target_vector = [qval(action1)     qval(action2)]
        ...
        if done:
            r = r + i
            if epi%10==0:     #Only for printing results after every 10 deaths (of cartpole)
                print epi, "Survived for ", i, "Average : ",1.0*r/10
                r = 0
            break
        state = new_state
    epi = epi + 1
